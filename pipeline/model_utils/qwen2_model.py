...
# Qwen2.5 chat templates are based on ChatML format
...
# Common refusal token IDs for Qwen2.5
# 'I' and 'As' are common starting tokens for refusal responses
QWEN2_REFUSAL_TOKS = [40, 2121]  # ['I', 'As']
...
def format_instruction_qwen2_chat(
    ...
    if system is not None:
        formatted_instruction = QWEN2_CHAT_TEMPLATE_WITH_SYSTEM.format(
...
def orthogonalize_qwen2_weights(model, direction: Float[Tensor, "d_model"]):
    ...
    Qwen2.5 uses Llama-style architecture:
    - model.model.embed_tokens for embeddings
    - model.model.layers for transformer blocks
    - self_attn.o_proj for attention output projection
    - mlp.down_proj for MLP output projection
    """
...
def act_add_qwen2_weights(model, direction: Float[Tensor, "d_model"], coeff, layer):
    """Add activation steering to the model at a specific layer."""
...
class Qwen2Model(ModelBase):
    """Model utility class for Qwen2.5 series models (e.g., Qwen/Qwen2.5-7B-Instruct)."""
    ...
    def _get_eoi_toks(self):
        return self.tokenizer.encode(
            QWEN2_CHAT_TEMPLATE.split("{instruction}")[-1], add_special_tokens=False
        )
    ...
    def _get_act_add_mod_fn(self, direction: Float[Tensor, "d_model"], coeff, layer):
        return functools.partial(
            act_add_qwen2_weights, direction=direction, coeff=coeff, layer=layer
        )
